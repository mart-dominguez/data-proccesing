...
def f1(mes,anio):
    ...
    df.to_csv('aud{}-{}.csv'.format(mes,anio))
    aud=pd.read_csv('aud{}-{}.csv'.format(mes,anio))
    ...
    # subset['Oferta'] = subset['Oferta'].astype(str)
    ...
    # dummy_oferta = pd.get_dummies(subset['Oferta'], prefix='Oferta')
    ...
    # subset=subset.rename({"Oferta_0":"NO_oferta"}, axis=1)
    # subset=subset.rename({"Oferta_1":"SI_oferta"}, axis=1)
    ...
    subset2.to_csv('busq_prueba{}-{}.csv'.format(mes,anio))
    print(subset2)
